<template name="MyTest">
	<div class="gesture-lock" :class="{error:error}" :style="{width: cWidth +'upx', height:cWidth +'upx'}"
	 @touchstart.stop="onTouchStart" @touchmove.stop="onTouchMove" @touchend.stop="onTouchEnd">
		<!-- 9 个圆 -->
		<div v-for="(item,i) in circleArray" :key="'info2-'+i" class="cycle" :class="{check:item.check}" :style="{left:item.style.left,top:item.style.top,width:item.style.width,height:item.style.width}">
		</div>
